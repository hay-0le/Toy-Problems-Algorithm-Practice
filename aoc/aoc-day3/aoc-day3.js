...
var wireIntersect = (path1, path2) => {
  ...
  var saveLineCoordinates = (endOfLine, move) => {
    var currentPosition = endOfLine;
    var lineCoordinates = [];
    var direction = move[0];
    var count = Number(move.slice(1));

    if (direction === "U") {
      //move up 1
      for (var i = 0; i < count; i++) {
        currentPosition[1] += 1;
        lineCoordinates.push(currentPosition.slice());
      }
    } else if (direction === "D") {
      //move down 1
      for (var i = 0; i < count; i++) {
  ...
  // var getLineCoordinates = (endOfLine, move) => {
  //   var lineCoordinates = {};
  ...
  //     for (var i = 0; i < count; i++) {
  //       currentPosition[1]++;
  //       var key = currentPosition[0].toString();
  //       lineCoordinates[key]: currentPosition[1]});
  //     }
  //   } else if (direction === "D") {
  //     //move down 1
  //     for (var i = 0; i < count; i++) {
  //       currentPosition[1]--;
  //       var key = currentPosition[0].toString();
  //       lineCoordinates.push({key: currentPosition[1]});
  //     }
  //   } else if (direction === "R") {
  //     //move right 1
  //     for (var i = 0; i < count; i++) {
  ...
  var path1Coordinates = getCoordinates(path1, saveLineCoordinates);
  var path2Coordinates = getCoordinates(path2, saveLineCoordinates);
  // console.log('path1Coordinates: ', path1Coordinates);
  // console.log('path2Coordinates: ', path2Coordinates);
  var leastSteps;

//part 2
 path1Coordinates.forEach((x, i) => {
    var step1 = i + 1;
    var run = x[0];
    var rise = x[1];

    path2Coordinates.forEach((y, idx) => {
      var step2 = idx + 1;

      if (y[0] === run && y[1] === rise) {

        var totalSteps = step1 + step2;
        if (totalSteps < leastSteps || !leastSteps) {
          console.log("new step count")
  ...
  // path1Coordinates.forEach(x => {
  //   var run = x[0];
  //   var rise = x[1];

  //   path2Coordinates.forEach(y => {
  //     if (y[0] === run && y[1] === rise) {

  //       var distance = Math.abs(rise) + Math.abs(run);

  //       if (closestIntersectionDistance === null) {
  //           closestIntersectionDistance = distance;
  //       } else if (distance < closestIntersectionDistance) {
  //           closestIntersectionDistance = distance;
  //           console.log("newest closest")
  //       }
  //     }
  //   })
  // })
...
return leastSteps;
  // console.log(closestIntersectionDistance)
  // return closestIntersectionDistance;

}

console.log("Closest Intersection Distance from central port: ", wireIntersect(wire1, wire2));
// console.log("Closest Intersection Distance from central port: ", wireIntersect(one, two));
